...
class TmdbRepository(private val api: TmdbApi) {
    ...
    suspend fun discoverMoviesPage(genreId: Int?, sortBy: String?, page: Int) =
        api.discoverMovies(genreId?.toString(), sortBy, page).toMediaResult("movie")
    suspend fun discoverTvPage(genreId: Int?, sortBy: String?, page: Int) =
        api.discoverTv(genreId?.toString(), sortBy, page).toMediaResult("tv")
    ...
    suspend fun searchPaged(query: String, page: Int = 1): PagedResponse<Media> = api.search(query, page)
